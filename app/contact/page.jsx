      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 rounded-3xl border border-white/50 bg-white/45 p-6 shadow-soft backdrop-blur-xl">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Fill everything — faster replies happen with clear context.
          </p>
          ...
          <form onSubmit={submit} className="mt-5 grid md:grid-cols-2 gap-4">
            ...
            <button
              ...
              className={`md:col-span-2 rounded-2xl px-5 py-3 text-sm font-medium text-white shadow-soft transition disabled:opacity-60 ${
                status === "success"
                  ? "bg-green-600"
            ...
            <div className="md:col-span-2 text-xs text-slate-500">
              Replies are stored in your Profile. If you’re not logged in, you
              can still send a message, but you won’t see replies on the site.
            </div>
          </form>
        </div>
        ...
        <aside className="rounded-3xl border border-white/50 bg-white/45 p-6 shadow-soft backdrop-blur-xl h-fit">
          ...
          <div className="mt-3 text-sm text-slate-600 space-y-2">
            <div>
              ...
              {contact.address || "Set in Admin → Settings"}
            </div>
            <div>
              <span className="font-medium text-slate-800">Email:</span>{" "}
              {contact.email || "hello@myseo.com"}
            </div>
            <div>
              <span className="font-medium text-slate-800">Phone:</span>{" "}
              {contact.phone || "+977-98XXXXXXXX"}
            </div>
          </div>
              ...
              {Object.entries(social)
                ...
                .map(([k, v]) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="rounded-full bg-white/60 px-3 py-2 text-sm text-slate-700 shadow-glass hover:bg-white/75 transition"
                  >
                    {k}
                  </a>
                ))}
          ...
          {contact.mapEmbedUrl && (
            <>
              <div className="text-sm font-semibold text-slate-900 mt-6">
                Find Us Here
              </div>
